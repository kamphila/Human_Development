Public Class Setup_Course_Item_POP
    Public mode As String = GBMod.View
    Public id As String = String.Empty
    Private Course_Item As New Cls_HD_Course_Item
    Public FormBrow As HD_System_Setup

    Private Sub Setup_Course_Item_POP_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        pmethBlankVar()
    ...
    Private Sub pmethGetdata()
        If String.IsNullOrEmpty(id) = False Then
            Course_Item.getById(id)
            txtCode.Text = Course_Item.code
            txtName.Text = Course_Item.name
            txtName2.Text = Course_Item.name2
            txtDesc.Text = Course_Item.desc
        End If
    End Sub
...
#Region "Event"
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        If ValidateData() = True Then
            Course_Item.Corp_id = GBCorp
            Course_Item.code = txtCode.Text
            Course_Item.name = txtName.Text
            Course_Item.name2 = txtName2.Text
            Course_Item.desc = txtDesc.Text
            If Course_Item.CheckDupCode = False Then
                MessageBox.Show("พบรหัส :" & Course_Item.code & " ในระบบแล้ว,กรุณาป้อนรหัสใหม่อีกครั้ง", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If

            If mode = GBMod.Add Then
                If Course_Item.Add = False Then
                    MessageBox.Show(Course_Item.message, "System Error :", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
            ElseIf mode = GBMod.Edit Then
                If Course_Item.Update() = False Then
                    MessageBox.Show(Course_Item.message, "System Error :", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
            End If
