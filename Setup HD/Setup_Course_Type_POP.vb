Public Class Setup_Course_Type_POP
    Public mode As String = GBMod.View
    Public id As String = String.Empty
    Private Course_Type As New Cls_HD_Course_Type
    Public FormBrow As HD_System_Setup

    Private Sub Setup_Course_Type_POP_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        pmethBlankVar()
    ...
    Private Sub pmethGetdata()
        If String.IsNullOrEmpty(id) = False Then
            Course_Type.getById(id)
            txtCode.Text = Course_Type.code
            txtName.Text = Course_Type.name
            txtName2.Text = Course_Type.name2
            txtDesc.Text = Course_Type.desc
        End If
    End Sub
...
#Region "Event"
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        If ValidateData() = True Then
            Course_Type.Corp_id = GBCorp
            Course_Type.code = txtCode.Text
            Course_Type.name = txtName.Text
            Course_Type.name2 = txtName2.Text
            Course_Type.desc = txtDesc.Text
            'Check Dupcate Code
            If Course_Type.CheckDupCode = False Then
                MessageBox.Show("พบรหัส :" & Course_Type.code & " ในระบบแล้ว,กรุณาป้อนรหัสใหม่อีกครั้ง", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If
            If mode = GBMod.Add Then
                If Course_Type.Add = False Then
                    MessageBox.Show(Course_Type.message, "System Error :", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
            ElseIf mode = GBMod.Edit Then
                If Course_Type.Update() = False Then
                    MessageBox.Show(Course_Type.message, "System Error :", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Exit Sub
                End If
            End If
