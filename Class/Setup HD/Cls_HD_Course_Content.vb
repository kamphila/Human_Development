...
Public Class Cls_HD_Course_Content
    Inherits Cls_Base
...
#Region "Function"
    ...
    Function getById(id As String) As Boolean
        ...
        Dim dt As DataTable
        dt = Retreive()
        var_corp_id = dt.Rows(0).Item("coc_corp_id").ToString()
        var_id = dt.Rows(0).Item("coc_id").ToString()
        var_code = dt.Rows(0).Item("coc_code").ToString()
        var_name = dt.Rows(0).Item("coc_name").ToString()
        var_name2 = dt.Rows(0).Item("coc_name2").ToString()
        var_desc = dt.Rows(0).Item("coc_desc").ToString()
        Return True
    End Function
    ...
    Function CheckDupCode() As Boolean
        Try
            ...
            strcommand = "select 1 from hd_course_content "
            If var_id.Length > 0 Then
                strwc = " and coc_id <> '" & var_id.ToString() & "'"
            End If
            strcommand = strcommand & vbCrLf & " where coc_corp_id = ? and coc_code = ? " + strwc
    ...
    Function Retreive() As DataTable
        Try
            ...
            End If
            strcommand = "select * "
            strcommand = strcommand & vbCrLf & "from hd_course_content where coc_corp_id = '" & GBCorp & "' " & strwc
            strcommand = strcommand & vbCrLf & "order by coc_code "
    ...
    Function Add() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "insert into hd_course_content(coc_corp_id,coc_id"
            strcommand = strcommand & vbCrLf & ",coc_code,coc_name,coc_name2,coc_desc,coc_create_by,coc_create_date)"
    ...
    Function Update() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "update hd_course_content"
            strcommand = strcommand & vbCrLf & "set coc_code = ?"
            strcommand = strcommand & vbCrLf & ",coc_name = ?"
            strcommand = strcommand & vbCrLf & ",coc_name2 = ?"
            strcommand = strcommand & vbCrLf & ",coc_Desc = ?"
            strcommand = strcommand & vbCrLf & ",coc_edit_by = ?"
            strcommand = strcommand & vbCrLf & ",coc_edit_date= ?"
            strcommand = strcommand & vbCrLf & "where coc_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
    ...
    Function Delete() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "delete from hd_course_content "
            strcommand = strcommand & vbCrLf & "where coc_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
