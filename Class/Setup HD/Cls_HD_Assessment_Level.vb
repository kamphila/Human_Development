...
Public Class Cls_HD_Assessment_Level
    Inherits Cls_Base
...
#Region "Function"
    ...
    Function getById(id As String) As Boolean
        ...
        Dim dt As DataTable
        dt = Retreive()
        var_corp_id = dt.Rows(0).Item("asl_corp_id").ToString()
        var_id = dt.Rows(0).Item("asl_id").ToString()
        var_code = dt.Rows(0).Item("asl_code").ToString()
        var_name = dt.Rows(0).Item("asl_name").ToString()
        var_name2 = dt.Rows(0).Item("asl_name2").ToString()
        var_desc = dt.Rows(0).Item("asl_desc").ToString()
        Return True
    End Function
    ...
    Function CheckDupCode() As Boolean
        Try
            ...
            strcommand = "select 1 from hd_assessment_level "
            If var_id.Length > 0 Then
                strwc = " and asl_id <> '" & var_id.ToString() & "'"
            End If

            strcommand = strcommand & vbCrLf & " where asl_corp_id = ? and asl_code = ? " + strwc
    ...
    Function Retreive() As DataTable
        Try
            ...
            End If
            strcommand = "select * "
            strcommand = strcommand & vbCrLf & "from hd_assessment_level where asl_corp_id = '" & GBCorp & "' " & strwc
            strcommand = strcommand & vbCrLf & "order by asl_code "
    ...
    Function Add() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "insert into hd_assessment_level(asl_corp_id,asl_id"
            strcommand = strcommand & vbCrLf & ",asl_code,asl_name,asl_name2,asl_desc,asl_create_by,asl_create_date)"
    ...
    Function Update() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "update hd_assessment_level"
            strcommand = strcommand & vbCrLf & "set asl_code = ?"
            strcommand = strcommand & vbCrLf & ",asl_name = ?"
            strcommand = strcommand & vbCrLf & ",asl_name2 = ?"
            strcommand = strcommand & vbCrLf & ",asl_Desc = ?"
            strcommand = strcommand & vbCrLf & ",asl_edit_by = ?"
            strcommand = strcommand & vbCrLf & ",asl_edit_date= ?"
            strcommand = strcommand & vbCrLf & "where asl_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
    ...
    Function Delete() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "delete from hd_assessment_level "
            strcommand = strcommand & vbCrLf & "where asl_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
