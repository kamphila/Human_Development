...
Public Class Cls_HD_Course_Type
    Inherits Cls_Base
...
#Region "Function"
    ...
    Function getById(id As String) As Boolean
        ...
        Dim dt As DataTable
        dt = Retreive()
        var_corp_id = dt.Rows(0).Item("cot_corp_id").ToString()
        var_id = dt.Rows(0).Item("cot_id").ToString()
        var_code = dt.Rows(0).Item("cot_code").ToString()
        var_name = dt.Rows(0).Item("cot_name").ToString()
        var_name2 = dt.Rows(0).Item("cot_name2").ToString()
        var_desc = dt.Rows(0).Item("cot_desc").ToString()
        Return True
    End Function
    ...
    Function CheckDupCode() As Boolean
        Try
            ...
            strcommand = "select 1 from hd_course_type "
            If var_id.Length > 0 Then
                strwc = " and cot_id <> '" & var_id.ToString() & "'"
            End If
            strcommand = strcommand & vbCrLf & " where cot_corp_id = ? and cot_code = ? " + strwc
    ...
    Function Retreive() As DataTable
        Try
            ...
            End If
            strcommand = "select * "
            strcommand = strcommand & vbCrLf & "from hd_course_type where cot_corp_id = '" & GBCorp & "' " & strwc
            strcommand = strcommand & vbCrLf & "order by cot_code "
    ...
    Function Add() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "insert into hd_course_type(cot_corp_id,cot_id"
            strcommand = strcommand & vbCrLf & ",cot_code,cot_name,cot_name2,cot_desc,cot_create_by,cot_create_date)"
    ...
    Function Update() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "update hd_course_type"
            strcommand = strcommand & vbCrLf & "set cot_code = ?"
            strcommand = strcommand & vbCrLf & ",cot_name = ?"
            strcommand = strcommand & vbCrLf & ",cot_name2 = ?"
            strcommand = strcommand & vbCrLf & ",cot_Desc = ?"
            strcommand = strcommand & vbCrLf & ",cot_edit_by = ?"
            strcommand = strcommand & vbCrLf & ",cot_edit_date= ?"
            strcommand = strcommand & vbCrLf & "where cot_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
    ...
    Function Delete() As Boolean
        ...
        Try
            SqlDB.BeginTrans()
            strcommand = "delete from hd_course_type "
            strcommand = strcommand & vbCrLf & "where cot_id = ? "
            cmd = SqlDB.CreateCommand(strcommand)
